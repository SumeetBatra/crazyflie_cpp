...
void Crazyflie::uploadNN(
  const std::vector<uint8_t>& data)
{
  for (const auto& entry : m_memoryTocEntries) {
    if (entry.type == MemoryTypeNN) {
      startBatchRequest();
      // upload data
      ...
      for (size_t i = 0; i < numRequests; ++i) {
        ...
        size_t size = std::min<size_t>(remainingBytes, 24);
        memcpy(req.data, reinterpret_cast<const uint8_t*>(data.data()) + i * 24, size);
        remainingBytes -= size;
        addRequestInternal(reinterpret_cast<const uint8_t*>(&req), 6 + size, 5);
      }
      handleRequests();
  ...
  throw std::runtime_error("Could not find MemoryTypeNN!");
}
